...
tStart = time.time()
print('Reading file: FOREX_key'+NAME1+', Time: ', int(time.time() - tStart),'s'+'\n')
KEY_DATA_t = readExcelFile(data_path+'FOREX_key'+NAME1+'.xlsx', header_ = 0, acceptNoFile=False, index_col_=0, sheet_name_='FOREX_key'+NAME1+'')
print('Reading file: FOREX_key'+NAME2+', Time: ', int(time.time() - tStart),'s'+'\n')
df_key = readExcelFile(data_path+'FOREX_key'+NAME2+'.xlsx', header_ = 0, acceptNoFile=False, index_col_=0, sheet_name_='FOREX_key'+NAME2+'')
#print('Reading file: FOREX_database, Time: ', int(time.time() - tStart),'s'+'\n')
#DATA_BASE_t = readExcelFile(data_path+'FOREX_database.xlsx', header_ = 0, index_col_=0, acceptNoFile=False)

print('Concating file: FOREX_key'+NAME1+', Time: ', int(time.time() - tStart),'s'+'\n')
KEY_DATA_t = pd.concat([KEY_DATA_t, df_key], ignore_index=True)
"""
print('Concating file: FOREX_database, Time: ', int(time.time() - tStart),'s'+'\n')
for d in DB_name_A:
